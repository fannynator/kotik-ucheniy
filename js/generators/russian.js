...
export function generateSoftLesson() {
    ...
    // 1. Разминка
    t.push(choiceStrT('🔥', 'Разминка', 'badge-warmup',
        `«${sel[0].word}» — Ь или Ъ?`, sel[0].ans, ['ь', 'ъ'], 0, sel[0].hint));
    ...
    // 3. Выбор
    t.push(choiceStrT('🎯', 'Выбор', 'badge-choice',
        `«${sel[2].word}» — какая буква?`, sel[2].ans, ['ь', 'ъ'], 0, sel[2].hint));
...
export function generateTsyaLesson() {
    ...
    t.push(choiceStrT('🔥', 'Разминка', 'badge-warmup',
        ...
        sel[0].ans, ['тся', 'ться'], 0, sel[0].hint));

    // 2. Визуальное — сравнение двух фраз
    t.push(visualT('🖼️', 'Визуальное', 'badge-visual',
        compareSVG('Он учит_ся', 'Надо учит_ся', 'Где ТСЯ, а где ТЬСЯ?'),
        'Он учится — ТСЯ, Надо учиться — ТЬСЯ',
        ['Он учится — ТСЯ, Надо учиться — ТЬСЯ', 'оба с ТЬСЯ', 'оба с ТСЯ', 'Надо учится — ТСЯ'],
        'Что делает? → ТСЯ. Что делать? → ТЬСЯ'));

    // 3. Выбор
    t.push(choiceStrT('🎯', 'Выбор', 'badge-choice',
        `«${sel[2].phrase}» — ?`, sel[2].ans, ['тся', 'ться'], 0, sel[2].hint));
...
export function generatePrepriLesson() {
    ...
    t.push(choiceStrT('🔥', 'Разминка', 'badge-warmup',
        ...
        sel[0].ans === 'е' ? 'ПРЕ' : 'ПРИ',
        ['ПРЕ', 'ПРИ'], 0, sel[0].hint));

    // 2. Визуальное
    t.push(visualT('🖼️', 'Визуальное', 'badge-visual',
        compareSVG('пр_бывать', 'пр_мудрый', 'Где ПРИ, а где ПРЕ?'),
        'прИбывать — ПРИ, прЕмудрый — ПРЕ',
        ['прИбывать — ПРИ, прЕмудрый — ПРЕ', 'оба с ПРЕ', 'оба с ПРИ', 'прЕбывать — ПРЕ'],
        'ПРИ = приближение, ПРЕ = очень'));

    // 3. Выбор
    t.push(choiceStrT('🎯', 'Выбор', 'badge-choice',
        `«${sel[2].word}» — ?`, sel[2].ans === 'е' ? 'ПРЕ' : 'ПРИ',
        ['ПРЕ', 'ПРИ'], 0, sel[2].hint));
...
export function generateNNLesson() {
    ...
    t.push(visualT('🖼️', 'Визуальное', 'badge-visual',
        ...
        'курИ-Н-ый → Н, солом-ЕНН-ый → НН',
        ['курИ-Н-ый → Н, солом-ЕНН-ый → НН', 'оба с НН', 'оба с Н', 'курИ-НН-ый → НН'],
        '-ИН- → Н, -ЕНН- → НН'));
