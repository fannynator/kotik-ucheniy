...
function appleSVG(count) {
    const size = 22, gap = 4, cols = Math.min(count, 6), rows = Math.ceil(count / cols);
    const w = cols * (size + gap) + 8, h = rows * (size + gap) + 8;
    ...
    for (let r = 0; r < rows; r++) for (let c = 0; c < cols && (r * cols + c) < count; c++) {
        const x = 6 + c * (size + gap), y = 6 + r * (size + gap);
...
function groupSVG(groups, items) {
    const itemSize = 18, gap = 4, cols = items, rows = groups;
    const w = cols * (itemSize + gap) + 20, h = rows * (itemSize + gap) + 20;
    let svg = `<svg width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" style="display:block;margin:8px auto;">`;
    for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
            const x = 12 + c * (itemSize + gap), y = 10 + r * (itemSize + gap);
            svg += `<circle cx="${x + itemSize/2}" cy="${y + itemSize/2}" r="${itemSize/2 - 2}" fill="#F59E0B" stroke="#D97706" stroke-width="1"/>`;
        }
    }
    return svg + `<text x="${w/2}" y="${h - 4}" text-anchor="middle" font-size="10" fill="#94A3B8">${groups}×${items}</text></svg>`;
}

function rectSVG(w, h) {
    const scale = 18, sw = w * scale + 30, sh = h * scale + 30;
...
function pizzaSVG(total, eaten) {
    let svg = `<svg width="120" height="120" viewBox="0 0 100 100" style="display:block;margin:8px auto;">
        <circle cx="50" cy="50" r="45" fill="#F59E0B" stroke="#D97706" stroke-width="2.5"/>`;
    for (let i = 0; i < total; i++) {
        const angle = (i / total) * 360 - 90, rad = (angle * Math.PI) / 180;
        svg += `<line x1="50" y1="50" x2="${50 + 43 * Math.cos(rad)}" y2="${50 + 43 * Math.sin(rad)}" stroke="rgba(255,255,255,0.4)" stroke-width="1"/>`;
    }
    for (let i = 0; i < eaten; i++) {
        const midAngle = ((i + 0.5) / total) * 360 - 90, rad = (midAngle * Math.PI) / 180;
        svg += `<circle cx="${50 + 28 * Math.cos(rad)}" cy="${50 + 28 * Math.sin(rad)}" r="2.5" fill="white" opacity="0.5"/>`;
    }
    return svg + `<text x="50" y="98" text-anchor="middle" font-size="9" fill="#94A3B8">${eaten}/${total}</text></svg>`;
}
