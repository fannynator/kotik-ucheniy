...
function getAudioContext() {
    if (!audioCtx) {
        try {
            ...
            // Если контекст в состоянии suspended — резюмируем при первом клике
...
export function spawnGems(count, startX, startY, targetEl) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        gem.textContent = '💎';
        gem.style.cssText = `position:fixed;left:${startX}px;top:${startY}px;font-size:16px;pointer-events:none;z-index:1000;transition:all 0.8s cubic-bezier(0.175,0.885,0.32,1.275);opacity:1;`;
